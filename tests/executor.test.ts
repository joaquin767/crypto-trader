...
import type { TradeSignal } from "../src/risk.ts";
...
test("execute 'hold' returns a hold result", async () => {
  const signal: TradeSignal = { type: "hold", symbol: "BTC/USDT", confidence: 1, reason: "test" };
...
test("execute 'buy' returns a buy result with quantity and fee", async () => {
  const signal: TradeSignal = { type: "buy", symbol: "BTC/USDT", confidence: 0.8, reason: "test" };
  const result = await execute(signal, config);
...
test("execute 'sell' returns a sell result", async () => {
  const signal: TradeSignal = { type: "sell", symbol: "BTC/USDT", confidence: 0.9, reason: "test" };
  const result = await execute(signal, config);
