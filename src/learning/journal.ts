...
// Exit attribution now lives in strategy/exit-reason.ts so the executor can
// use it without closing an import cycle (journal -> executor -> journal).
// Re-exported here because existing callers import it from the journal.
export type { ExitReason, ExitReasonBucket } from "../strategy/exit-reason.ts";
export { classifyExitReason } from "../strategy/exit-reason.ts";
